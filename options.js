...
function formatGmtLabel(label) {
  const gmtTextFull = label.slice(1, 10);

  if (gmtTextFull.slice(7) === "00") {
    return gmtTextFull.slice(0, 6);
  } else {
    return gmtTextFull;
  }
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  for (let i = 0; i < timezonesList.length; i++) {
    ...
    option.value = timezonesList[i].value;
    option.innerHTML = timezonesList[i].label;
    timezoneSelect.appendChild(option);
  }

  //load saved timezones
  chrome.storage.sync.get("nttmTimezones", (res) => {
    let ul = document.getElementById("timezones");
    if (res.nttmTimezones) {
      for (let i = 0; i < res.nttmTimezones.length; i++) {
        let timezone = res.nttmTimezones[i];
        savedTimezones.push(timezone);

        const gmt = timezonesList.find((tz) => tz.value === timezone.timezone);
        const gmtTextFormatted = formatGmtLabel(gmt.label);
        let li = document.createElement("li");
        li.innerHTML = `(${gmtTextFormatted}) ${timezone.timezone} <span class="removeBtn" data-timezone="${timezone.timezone}">&#10006;</span>`;
        ul.appendChild(li);
      }
      ...
      if (removeBtns.length > 0) {
        Array.from(removeBtns).forEach((removeBtn) => {
          removeBtn.addEventListener("click", (el) => {
            let timezone = el.target.dataset.timezone;

            let filteredTimezones = savedTimezones.filter(
              (tz) => tz.timezone !== timezone
            );
            chrome.storage.sync.set({ nttmTimezones: filteredTimezones });
            chrome.extension.getViews()[0].location.reload();
          });
        });
      }
    } else {
      let li = document.createElement("li");
      li.innerHTML = "No timezones saved";
      ul.appendChild(li);
    }
  });
  ...
  addBtn.addEventListener("click", () => {
    ...
    const gmt = timezonesList.find((tz) => tz.value === timezone);
    const gmtTextFormatted = formatGmtLabel(gmt.label);

    savedTimezones.push({ timezone: timezone, gmt: gmtTextFormatted });
    chrome.storage.sync.set({ nttmTimezones: savedTimezones });
    chrome.extension.getViews()[0].location.reload();
  });
});
